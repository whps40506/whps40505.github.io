...
var music = ["Luka_bgm1.mp3", "Luka_bgm2.mp3", "Luka_bgm3.mp3"];
...
function pause() {
    if (Game.playing) {
        ...
        Game.playing = true;
        document.getElementById("pause").setAttribute("value", "BUG 召喚器");
        enemy_moveControl();
    }
}
...
function run() {
    ...
    if (Keyboard.down && player.y < 940 - 37-32) {
        player.y += player.speed;
    }
    if (Keyboard.left && player.x > 0) {
        player.x -= player.speed;
    }
    if (Keyboard.right && player.x < 800 - 37-32) {
        player.x += player.speed;
    }
    ...
    // 碰撞發生時執行(傷害判定)
    if (Matrix.drawplayer(player.x, player.y)) {
        // 損血並即時更新血量
        player.hp--;
        document.getElementById("HP").innerText = "HP:" + player.hp;
        // 血量歸零時結束遊戲
...
function enemy_atkControl(delay) {
    if (!enemy.moving) {
        switch (delay) {
            ...
            // 高速泡泡
            case 400:
                new RoundDirectAmmo(enemy.x - 5, enemy.y - 5, 20, 12).queue3();
                break;
            ...
            case 5000:
                ...
                for (var i = 0; i < 30; i++) {
                    ...
                    setTimeout(function () { offset +=70; new RoundDirectAmmo(enemy.x + 18, enemy.y + 18, 18, 10, offset).queue() }, 150 * (i + 1));
